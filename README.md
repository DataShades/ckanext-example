[![Tests](https://github.com/DataShades/ckanext-example/workflows/tests.yml/badge.svg)](https://github.com/DataShades/ckanext-example/actions/workflows/test.yml)
...
Code of the extension resides inside `ckanext/example`.
...
| Blanket             | Effect                                                                                              |
|---------------------|-----------------------------------------------------------------------------------------------------|
| actions             | Register all public functions from `ckanext.example.logic.action` as actions        |
| auth_functions      | Register all public functions from `ckanext.example.logic.auth` as auth functions   |
| blueprints          | Register all blueprints from `ckanext.example.views` as blueprints                  |
| cli                 | Register all public members(`__all__`) from `ckanext.example.cli` as commands       |
| config_declarations | Register all declarations from `ckanext/example/config_declaration.yaml`            |
| helpers             | Register all public functions from `ckanext.example.helpers` as helpers             |
| validators          | Register all public functions from `ckanext.example.logic.validators` as validators |
...
function defined inside `ckanext.example.logic.action` will be
...
First, create a module inside `ckanext.example.implementations`
...
Re-export implementation from `ckanext/example/implementations/__init__.py`
...
the module. As result, only this group will be available as `ckan example` CLI command.
...
* config options can be accessed by shorter name: `option()` instead of
  `tk.config["ckanext.example.option.name"]`.
...
`assets/webassets.yml` and include this named asset into template using `{% asset "example/ASSET_NAME" %}` tag.
...
template for the blog page may be stored as `templates/example/blog/index.html`.
...
to generate migrations for the model using `ckan generate migration -p example -m "Migration message"` CLI command.
...
so we suggest leaving them inside `ckanext/example/schemas`.
...
`ckanext/example/tests` contains examples of tests for standard
...
`ckanext/example/tests/benchmarks` contains benchmarks. They
...
pytest --cov=ckanext.example

## generate HTML report at htmlcov/index.html
pytest --cov=ckanext.example --cov-report html
```
...
Run tests from `ckanext/example/tests/test_plugin.py`

```sh
pytest ckanext/example/tests/test_plugin.py
```
...
contains filepath, class and test name: `ckanext/example/tests/test_smth.py:TestSmth:test_smth`
