"""CLI commands for ckanext-example.
...
@click.pass_context
def example(ctx: click.Context):
    """CLI commands of example plugin."""
    ctx.meta["example"] = "example"


# Command decorated with `example.command()` decorator is
...
def command(ctx: click.Context, name: str, verbose: int):
    ...
    msg = f"Hello, {name or ctx.meta['example']}"
    if verbose:
        msg += "!" * verbose
