"""Tests for ckanext.example.cli module.
...
import ckanext.example.cli as commands
...
class TestCommand:
    ...
    def test_output(self, cli: CliRunner, faker: Faker):
        ...
        # second argument of `cli.invoke`. For example, `example
        # a b -C 1 --xxx=2` becomes `cli.invoke(example, ["a",
        # "b", "-C", "1", "--xxx=2"])`
        result = cli.invoke(commands.example, ["command"])
        # `result.exit_code` contains `0` if command succeeded
        assert not result.exit_code
        # `result.output` contains normal output. `result.stderr` contains
        # output of an error
        assert result.output == "Hello, example\n"

        # use fake data instead of hardcoded test values to discover
        # output-based problems
        name = faker.word()
        result = cli.invoke(commands.example, ["command", name])
        assert result.output == f"Hello, {name}\n"

    def test_verbosity(self, cli: CliRunner):
        """Verbosity level appends exclamation marks."""
        result = cli.invoke(commands.example, ["command", "-v"])
        assert result.output == "Hello, example!\n"

        result = cli.invoke(commands.example, ["command", "-vvv"])
        assert result.output == "Hello, example!!!\n"
...
class TestCountUsers:
    ...
    def test_output(self, cli: CliRunner, faker: Faker, user_factory: Any):
        ...
        result = cli.invoke(commands.example, ["count-users"])
