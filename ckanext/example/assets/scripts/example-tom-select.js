 ...
 */
ckan.module("example-tom-select", function () {
  ...
  return {
    ...
    // `this.sandbox.example.nestedOptions` and we receive
    // `{helloWorld: {byeWorld: ...}}`.
    options: {
    ...
    initialize() {
      ...
      if (typeof TomSelect === "undefined") {
        ...
        console.error("[example-tom-select] TomSelect library is not loaded");
        return
      }

      // tom-select has a number of nested options. We are using
      // `nestedOptions` helper defined inside `example.js` to
      // convert flat options of CKAN JS module into nested object.
      const options = this.sandbox["example"].nestedOptions(this.options);
